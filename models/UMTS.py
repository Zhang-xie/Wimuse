...
# the implementation of loss weighting MTL model
...
class AutomaticWeightedLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, loss in enumerate(x):
            ...
            loss_sum += 1.0 / (self.params[i] ** 2) * loss + torch.log(self.params[i])
            # based on Auxiliary tasks in multi-task learning:
            # loss_sum += 0.5 / (self.params[i] ** 2) * loss + torch.log(1 + self.params[i] ** 2)
        return loss_sum


class UMTS(nn.Module):
    def __init__(self,inchannel_SE, groups_SE, layers_DE, strides_DE, block_DE, inchannel_DE, inchannel_Cl,
                 num_categories):
        super(UMTS, self).__init__()
...
class UMTS_PL(pl.LightningModule):
    def __init__(self,inchannel,group,num_categories,task_index):
        ...
        self.model = UMTS(inchannel_SE=inchannel, groups_SE=group, layers_DE=[1,2], strides_DE=[1,2],block_DE=BasicBlock,
                         inchannel_DE=128*group, inchannel_Cl=128*group,num_categories=num_categories)
        self.awl = AutomaticWeightedLoss(len(num_categories))
...
if __name__ == "__main__":
    ...
    basic = UMTS(inchannel_SE=52, groups_SE=1, layers_DE=[1,2], strides_DE=[1,2],block_DE=BasicBlock,
